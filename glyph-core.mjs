...
export function renderSegments(group, segments, stroke = DEFAULT_STROKE) {
  segments.forEach((segment) => {
    if (segment.visible === false) {
      return;
    }
    ...
    if (segment.kind === "quadratic") {
      const path = [
        `M${segment.start[0]},${segment.start[1]}`,
        `Q${segment.control[0]},${segment.control[1]} ${segment.end[0]},${segment.end[1]}`
      ].join(" ");
      group.path(path)
        .fill("none")
        .stroke(stroke)
    ...
    if (segment.kind === "arc") {
      ...
      const path = [
        `M${start[0]},${start[1]}`,
        `A${segment.radii[0]},${segment.radii[1]} ${segment.rotation || 0} ${largeArcFlag},${sweepFlag} ${end[0]},${end[1]}`
      ].join(" ");
      group.path(path)
        .fill("none")
        .stroke(stroke)
...
export function renderGlyphNode(node, group, options = {}) {
  ...
  renderSegments(group, fitted, options.stroke || DEFAULT_STROKE);
  return fitted;
}
